package net.zam.hopandharvest.item;
    ...
    public ItemStack finishUsingItem(ItemStack item, Level world, LivingEntity entity) {
        if (entity instanceof Player player) {
            if (player.getItemInHand(InteractionHand.MAIN_HAND).isEmpty())
                player.setItemInHand(InteractionHand.MAIN_HAND, new ItemStack(ZAMItems.MUG_EMPTY.get()));
            else if (!player.getInventory().add(new ItemStack(ZAMItems.MUG_EMPTY.get())))
                player.drop(new ItemStack(ZAMItems.MUG_EMPTY.get()), false);
        }
