package net.zam.hopandharvest.block.plants;
...
import net.zam.hopandharvest.registry.ZAMBlocks;
...
public class HopPlant extends CaveVinesPlantBlock {
    ...
    protected GrowingPlantHeadBlock getHeadBlock() {
        return (GrowingPlantHeadBlock) ZAMBlocks.HOP.get();
    }

    @Override
    public ItemStack getCloneItemStack(BlockState state, HitResult target, BlockGetter level, BlockPos pos, Player player) {
        return new ItemStack(ZAMItems.HOP.get());
    }
    ...
    public InteractionResult use(BlockState state, Level world, BlockPos pos, Player player, InteractionHand hand, BlockHitResult result) {
        if (state.getValue(BERRIES)) {
            Block.popResource(world, pos, new ItemStack(ZAMItems.HOP.get(), 1));
